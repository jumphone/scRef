...
load('tsnealign_pbmc.RObj')
library(Seurat)
library(SingleCellExperiment)
library(scmap)
#source('scRef.R')

tmp=as.matrix(as.factor(pbmc@meta.data$orig.ident))
#tmp[,1]=as.factor(tmp[,1])
rownames(tmp)=colnames(pbmc@data)
colnames(tmp)="cell_type1"
ann=tmp
...
set.seed(123)
sim_exp_sc_mat=exp_sc_mat
sim_ann=as.matrix(rep('NK.cell',ncol(sim_exp_sc_mat)))
rownames(sim_ann)=colnames(sim_exp_sc_mat)
colnames(sim_ann)='cell_type1'
